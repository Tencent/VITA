...
if true
then
	load_dir="${ROOT_PATH_4}/models/google/siglip-so400m-patch14-384/"
	save_dir="${ROOT_PATH_4}/models/google/siglip-so400m-patch14-384_tp8pp1"
	rm -fr ${save_dir}
	mkdir -p ${save_dir}
	python3 ${CODE_PATH}/lcvlm_modellink/clip_converter_siglip.py --download-root ${load_dir} --output ${save_dir} --tensor-parallel-size 8
