...
class VicunaDataset(BaseDataset):
    def __init__(
        ...
        self.system_message = "A chat between a curious user and an artificial intelligence assistant. The assistant gives helpful, detailed, and polite answers to the user's questions."
...
def preprocess(
    ...
    BOS_ID = tokenizer(" ", add_special_tokens=False).input_ids
    EOS_ID = tokenizer("</s>", add_special_tokens=False).input_ids

    USER_ID = tokenizer("USER: ", add_special_tokens=False).input_ids
    ASSISTANT_ID = tokenizer("ASSISTANT: ", add_special_tokens=False).input_ids
    ...
    for i, source in enumerate(sources):
        ...
        if system_message is not None and len(system_message) > 0:
            system = tokenizer(system_message, add_special_tokens=False).input_ids
            input_id += system
        ...
        for j, sentence in enumerate(source):
            ...
            if role == "user":
                ...
                _input_id = (
                    ASSISTANT_ID + tokenizer(value, add_special_tokens=False).input_ids + EOS_ID
                )
                _target = (
                    [IGNORE_TOKEN_ID] * len(ASSISTANT_ID)
                    + tokenizer(value, add_special_tokens=False).input_ids
                    + EOS_ID
                )
