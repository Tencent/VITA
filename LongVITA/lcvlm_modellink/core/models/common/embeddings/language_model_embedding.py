...
class LanguageModelEmbedding(MegatronModule):
    ...
    def forward(self, input_ids: Tensor, position_ids: Tensor, tokentype_ids: int = None, external_feature_dict: dict = None) -> Tensor:
        ...
        if external_feature_dict is not None:
            ...
            # print(f"external_feature_dict {external_feature_dict.keys()}")
            # print(f"word_embeddings {word_embeddings.size()}")
            # print(f"features {features.size()}")
            if 'indices' in external_feature_dict:
                # print(f"indices {external_feature_dict['indices'].size()}")
                indices_b, indices_s = external_feature_dict['indices'].unbind(dim=0)
                # print(f"indices_b {indices_b.size()}")
                # print(f"indices_s {indices_s.size()}")
                # print(word_embeddings[indices_b.view(-1), indices_s.view(-1)].size())
                # print(features.view(-1, features.shape[-1]).size())
                # print(word_embeddings[indices_b.view(-1), indices_s.view(-1)])
                # print(features.view(-1, features.shape[-1]))
