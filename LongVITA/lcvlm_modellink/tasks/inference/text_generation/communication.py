...
def recv_from_prev_pipeline_rank_(recv_buffer=None):
    ...
    if not parallel_state.is_pipeline_first_stage():
        ensure_var_is_not_none(recv_buffer)
...
def _is_cuda(tensor):
    ...
    if tensor is None or not tensor.is_cuda:
        raise ValueError("Check if a tensor is not none and is cuda.")
...
def _is_cuda_contiguous(tensor):
    ...
    if not tensor.is_contiguous():
        raise ValueError("Check if a tensor is not none, is cuda, and is contiguous.")
...
def broadcast_from_last_pipeline_stage(size, dtype, tensor=None):
    ...
    if parallel_state.is_pipeline_first_stage() and is_last_stage:
        return tensor
