...
from vl_load.vita.constants import AUDIO_TOKEN_INDEX, IGNORE_INDEX, IMAGE_TOKEN_INDEX
...
class VITAMetaForCausalLM(ABC):
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        ), (input_ids, sum([(cur == AUDIO_TOKEN_INDEX).sum() for cur in input_ids]), sum([(AUDIO_TOKEN_INDEX not in cur) for cur in input_ids]), audio_features["inputs_embeds"].shape[0])
