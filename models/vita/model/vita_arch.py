...
from ..constants import AUDIO_TOKEN_INDEX, IGNORE_INDEX, IMAGE_TOKEN_INDEX, ACTION_TOKEN_INDEX, STATE_TOKEN_INDEX

from .multimodal_encoder.builder import build_audio_encoder, build_vision_tower
from .multimodal_projector.builder import build_vision_projector
import numpy as np

# vitameta model, including vision tower, audio encoder, and projectors
class VITAMetaModel:
            ...
            for file_name in os.listdir(model_args.model_name_or_path):
                if file_name.endswith('safetensors'):
                    audio_weights.update(
                        {k[20:]: v 
                         for k, v in load_file(os.path.join(
                             model_args.model_name_or_path, file_name)).items() if
...
class VITAMetaForCausalLM(ABC):
    ...
    def set_action_idx(self, input_ids, action_token_idx=ACTION_TOKEN_INDEX):
        self.action_indices = torch.where(input_ids == action_token_idx)

        # 填充后一个<image>变为 image_num_token数量
        idx = 0
        batch_idx = list(set(self.action_indices[0].tolist()))
        for i in range(len(batch_idx)):
            for j in range((self.action_indices[0]==batch_idx[i]).sum()):
                self.action_indices[1][idx] += (self.image_token_num - 1)*2*(j+1)
                idx += 1
    ...
    # the inputs prepare function for VLA model
    def prepare_inputs_labels_for_vla(
        self, input_ids, position_ids, attention_mask, 
        past_key_values, labels, images, audios, state_embedding, 
        action_pred_token, action_pred_steps, sf_masks, shared_v_pid_stride=None
    ):
        
        vision_tower = self.get_vision_tower()
        action_indices = get_action_indices(input_ids, self.image_token_num, action_pred_steps)
        
        # import pdb;pdb.set_trace()
        
        # # 定义替换的值
        # replacement = torch.tensor([-200] * 49)
        ...
        #     for value in row:
        #         if value == -200:
        #             new_row.extend(replacement.tolist())
        #         else:
        #             new_row.append(value)
        #     output_ids.append(new_row)

        # # 将结果转换为张量
        # output_ids = [torch.tensor(row) for row in output_ids]
        ...
        # import pdb;pdb.set_trace()
        state_embedding = list(state_embedding.view(-1, 1, state_embedding.shape[-1]))
        ...
        assert (
            ...
            == len(image_features)
        ), (input_ids, len(image_features), 
            sum([(cur == IMAGE_TOKEN_INDEX).sum() for cur in input_ids]), 
            sum([(IMAGE_TOKEN_INDEX not in cur) for cur in input_ids]))

        for batch_idx, cur_input_ids in enumerate(input_ids):
            num_images = (cur_input_ids == IMAGE_TOKEN_INDEX).sum()
            num_action = (cur_input_ids == ACTION_TOKEN_INDEX).sum()
            num_state = (cur_input_ids == STATE_TOKEN_INDEX).sum()

            image_action_state_token_indices = (
                [-1]
                + torch.where(
                    (cur_input_ids == IMAGE_TOKEN_INDEX) | 
                    (cur_input_ids == ACTION_TOKEN_INDEX) | 
                    (cur_input_ids == STATE_TOKEN_INDEX)
                )[0].tolist()
            ...
            for i in range(len(image_action_state_token_indices) - 1):
                cur_input_ids_noim_noau.append(
                    cur_input_ids[
                        image_action_state_token_indices[i] + 1 : image_action_state_token_indices[i + 1]
                    ]
                )
                cur_labels_noim_noau.append(
                    cur_labels[image_action_state_token_indices[i] + 1 : image_action_state_token_indices[i + 1]]
                )
            ...
            for i in range(num_images + num_action + num_state + 1):
                cur_new_input_embeds.append(cur_input_embeds_no_im_no_au[i])
                cur_new_labels.append(cur_labels_noim_noau[i])
                if i < num_images + num_action + num_state:
                    if cur_input_ids[image_action_state_token_indices[i + 1]] == IMAGE_TOKEN_INDEX:
                    ...
                    elif cur_input_ids[image_action_state_token_indices[i + 1]] == ACTION_TOKEN_INDEX:
                        # import pdb;pdb.set_trace()
                        # cur_action_features = action_pred_token[cur_action_idx % 60]
                        cur_action_features = action_pred_token
                        # cur_action_idx += 1 
                        # for i in cur_new_input_embeds:
                        #     cur_action_idx += i.shape[0]
                        cur_new_input_embeds.append(cur_action_features)
                        cur_new_labels.append(
                            torch.full(
                                (cur_action_features.shape[0],),
                                IGNORE_INDEX,
                                device=cur_labels.device,
                                dtype=cur_labels.dtype,
                            )
                        )
                    elif cur_input_ids[image_action_state_token_indices[i + 1]] == STATE_TOKEN_INDEX:
                        cur_state_embedding = state_embedding[cur_state_idx]
                        cur_state_idx += 1
                        cur_new_input_embeds.append(cur_state_embedding)
                        cur_new_labels.append(
                            torch.full(
                                (cur_state_embedding.shape[0],),
                                IGNORE_INDEX,
                                device=cur_labels.device,
                                dtype=cur_labels.dtype,
                            )
        ...
        assert cur_state_idx == len(state_embedding) 
            ...
            if getattr(self.config, "tokenizer_padding_side", "right") == "left":
                ...
                if cur_len > 0:
                    ...
                    if shared_v_pid_stride is None:
                        ...
                        cur_shared_position_ids = make_shared_position_ids(
        ...
        if _attention_mask is None:
            ...
            attention_mask = attention_mask.to(dtype=_attention_mask.dtype, device=self.device)

        if _position_ids is None and shared_v_pid_stride is None:
            position_ids = None
 
        return None, position_ids, attention_mask, past_key_values, new_input_embeds, new_labels, action_indices
    
    # # the prepare function for VLM
    # def prepare_inputs_labels_for_vlm(
    #     self, input_ids, position_ids, attention_mask, 
    #     past_key_values, labels, images, audios, sf_masks, shared_v_pid_stride=None
    # ):
    #     vision_tower = self.get_vision_tower()
    #     if vision_tower is None or images is None or input_ids.shape[1] == 1:
    #         if (
    ...
    #         ), (input_ids, sum([(cur == AUDIO_TOKEN_INDEX).sum() for cur in input_ids]),  
    #             sum([(cur == -101).sum() for cur in new_labels]), sum([(cur == -102).sum() for cur in new_labels]), 
    #             new_labels.shape)
...
# 这里的image_token_num，如果分辨率是200，那么是49，如果是224，就是64

def get_action_indices(input_ids, image_token_num = 49, action_pred_steps = 3, action_token_idx=ACTION_TOKEN_INDEX):
    action_indices = torch.where(input_ids == action_token_idx)
    #embedding替换以后，一个<image>变为 image_num_token数量, 所以需要按照顺序找出对应的action位置
    idx = 0
    
    # import pdb; pdb.set_trace()
    batch_idx = list(set(action_indices[0].tolist()))
    for i in range(len(batch_idx)):
        for j in range((action_indices[0]==batch_idx[i]).sum() // action_pred_steps):
            for _ in range(action_pred_steps):
                action_indices[1][idx] += (image_token_num - 1)*2*(j+1)
                idx += 1
...
def merge_consecutive_tuples(tuples_list):
    """
    合并一个列表中连续或重叠的元组（表示区间）。
    例如：[(1, 5), (6, 10), (12, 15)] -> [(1, 10), (12, 15)]
    """
    if not tuples_list:
        return []
...
def make_shared_position_ids(cur_v_start_end, cur_len, shared_v_pid_stride):
        """
    为视频帧创建共享的位置ID。
    使得来自同一帧的多个 patch token 具有相同或相似的位置ID。
    """
