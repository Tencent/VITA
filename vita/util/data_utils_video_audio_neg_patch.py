...
def _get_rawvideo_dec(video_path, image_processor, max_frames=32, min_frames=4, image_resolution=384, video_framerate=1, s=None, e=None, image_aspect_ratio='pad'):
                    ...
                    for k, img in enumerate(image):
                        if set_id[k] not in NoPatchSets:
                            img, p_num = dynamic_preprocess(img, min_num=self.data_args.min_dynamic_patch,
                                                    ...
                                                    image_size=image_size, use_thumbnail=self.data_args.use_thumbnail)
                        else:
                            img, p_num = [img], [1]
                        image_patches += img
                        patch_num += p_num
