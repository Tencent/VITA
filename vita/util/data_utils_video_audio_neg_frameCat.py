...
from vita.constants import IGNORE_INDEX, DEFAULT_IMAGE_TOKEN, MAX_IMAGE_LENGTH, MIN_IMAGE_LENGTH, DEFAULT_VIDEO_TOKEN, DEFAULT_AUDIO_TOKEN, DEFAULT_DATA_RATIO
...
from vita.config import DataConfig, FolderDict, AudioFolder, NoPatchSets
import random
import math
...
def preprocess_multimodal(
    ...
    for source in sources:
        ...
        for i, sentence in enumerate(source):
            ...
            replace_token, vid_replace_token, audio_replace_token = DEFAULT_IMAGE_TOKEN, DEFAULT_IMAGE_TOKEN * image_token_num, DEFAULT_AUDIO_TOKEN# * audio_lens
...
def preprocess_mixtral_zh(
    ...
    if has_image and not has_audio:
        ...
        input_ids = torch.stack(
            [tokenizer_image_audio_token(prompt, tokenizer, return_tensors='pt') for prompt in conversations], dim=0)
...
def preprocess(
    ...
    if conversation_lib.default_conversation.version == "mixtral_zh":
        return preprocess_mixtral_zh(sources, tokenizer, has_image=has_image, has_audio=has_audio, end_tag=end_tag)
    elif conversation_lib.default_conversation.version == "mixtral_two":
        return preprocess_mixtral_two(sources, tokenizer, has_image=has_image, has_audio=has_audio, end_tag=end_tag, modality=modality)


def _get_rawvideo_dec(video_path, image_processor, max_frames=MAX_IMAGE_LENGTH, min_frames=MIN_IMAGE_LENGTH, image_resolution=384, video_framerate=1, s=None, e=None, image_aspect_ratio='pad'):
    ...
    if num_frames > 0:
        ...
        t_stride = int(round(float(fps) / sample_fps))
        all_pos = list(range(f_start, f_end + 1, t_stride))
        sample_pos = [all_pos[_] for _ in np.linspace(0, len(all_pos) - 1, num=num_frame, dtype=int)]
        ...
        for i in range(0, len(patch_images), 4):
            ...
            new_image = Image.new(patch_images[0].mode, (2 * width, 2 * height), tuple(int(x * 255) for x in image_processor.image_mean))
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        if 'image' in sources[0] and "audio" not in sources[0]:
            ...
            if type(image_file) is list:
                ...
                image = [Image.open(os.path.join(self.folder_dict[set_id[k]], file.replace("\\", "/"))).convert('RGB') for k, file in enumerate(image_file)]
                if self.data_args.image_aspect_ratio == 'pad':
                    ...
                    for k, img in enumerate(image):
                        if set_id[k] not in NoPatchSets:
                            img, p_num = dynamic_preprocess(img, min_num=self.data_args.min_dynamic_patch,
                                                    max_num=self.data_args.max_dynamic_patch,
                                                    image_size=image_size, use_thumbnail=self.data_args.use_thumbnail, img_mean=processor.image_mean)
                        else:
                            img, p_num = [img], [1]
                        image_patches += img
                        patch_num += p_num
                ...
                if self.data_args.image_aspect_ratio == 'pad':
                    ...
                    image, patch_num = dynamic_preprocess(image, min_num=self.data_args.min_dynamic_patch,
                                            max_num=self.data_args.max_dynamic_patch,
                                            image_size=image_size, use_thumbnail=self.data_args.use_thumbnail, img_mean=processor.image_mean)
                    image = [processor.preprocess(i, return_tensors='pt')['pixel_values'][0] for i in image]
                else:
            ...
            if type(audio_file) is list:
                ...
                for file in audio_file:
                    try:
                        a, a_llm = self.data_args.audio_processor.process(os.path.join(audio_folder, 'audio', file))
                    except:
                ...
                audio, audio_for_llm_lens = self.data_args.audio_processor.process(os.path.join(audio_folder, 'audio', audio_file))
            ...
            image, image_token_num = _get_rawvideo_dec(os.path.join(video_folder, video_file), processor, max_frames=MAX_IMAGE_LENGTH, min_frames=MIN_IMAGE_LENGTH, image_resolution=image_size, image_aspect_ratio=self.data_args.image_aspect_ratio)
...
def dynamic_preprocess(image, min_num=1, max_num=6, image_size=448, use_thumbnail=False, img_mean=0):
    ...
    resized_img = expand2even(resized_img, new_target_width, new_target_height, tuple(int(x*255) for x in img_mean))
...
def visualize_images(resized_img, processed_images, output_path='output.png'):
    ...
    fig, axes = plt.subplots(nrows=(len(processed_images) // 5) + 1, ncols=5, figsize=(15, (len(processed_images) // 5) + 1))
