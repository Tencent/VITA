        ...
        for key in model_dict.keys():
            if key in checkpoint.keys():
                if model_dict[key].shape == checkpoint[key].shape:
                    ...
                    print('Key {} has different shape, {} VS {}'.format(key, model_dict[key].shape, checkpoint[key].shape))
            else:
...
class VITAMetaForCausalLM(ABC):
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        assert sum([(cur == IMAGE_TOKEN_INDEX).sum() for cur in input_ids]) + sum([(IMAGE_TOKEN_INDEX not in cur) for cur in input_ids]) == image_features.shape[0]
        ...
        for batch_idx, cur_input_ids in enumerate(input_ids):
            ...
            image_audio_token_indices = [-1] + torch.where((cur_input_ids == IMAGE_TOKEN_INDEX) | (cur_input_ids == AUDIO_TOKEN_INDEX))[0].tolist() \
                                     + [cur_input_ids.shape[0]]
            cur_input_ids_noim_noau = []
            cur_labels = labels[batch_idx]
            cur_labels_noim_noau = []
            for i in range(len(image_audio_token_indices) - 1):
                cur_input_ids_noim_noau.append(cur_input_ids[image_audio_token_indices[i] + 1:image_audio_token_indices[i + 1]])
                cur_labels_noim_noau.append(cur_labels[image_audio_token_indices[i] + 1:image_audio_token_indices[i + 1]])
            ...
            for i in range(num_images + num_audio_frames + 1):
                ...
                if i < num_images + num_audio_frames:
                    if cur_input_ids[image_audio_token_indices[i+1]] == IMAGE_TOKEN_INDEX:
                        ...
                        cur_new_labels.append(
                            torch.full((cur_audio_features.shape[0],), IGNORE_INDEX, device=cur_labels.device,
                                    dtype=cur_labels.dtype))
                    else:
                        raise ValueError
