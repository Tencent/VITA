...
class RelPositionalEncoding(PositionalEncoding):
    ...
    def infer(self, xs, pe_index):
        ...
        position = torch.arange(pe_index, pe_index + self.full_chunk_size,
                                dtype=torch.float32).unsqueeze(1)
...
class MultiHeadedAttention(nn.Module):
    ...
    def infer(self, query, key, value, pos_emb, buffer, buffer_index, buffer_out):
        ...
        key_value_buffer = buffer[buffer_index: buffer_index + self.key_buffer_size + self.value_buffer_size].reshape([1, self.h, self.buffersize * 2, self.d_k])
        ...
        buffer_out.append(torch.cat([key_buffer[:, :, self.chunk_size:, :], value_buffer[:, :, self.chunk_size:, :]], dim=2).reshape(-1))
        ...
        if self.rel_enc:
            ...
            scores = torch.matmul(q, key_buffer.transpose(-2, -1) ) / math.sqrt(self.d_k)  # (batch, head, len_q, buffersize)
...
class SoftAttention(nn.Module):
    def __init__(self, in_dim, hidden_dim):        
        ...
        self.buffer[:,:] = float(numpy.finfo(torch.tensor(0, dtype=torch.float32).numpy().dtype).min)
