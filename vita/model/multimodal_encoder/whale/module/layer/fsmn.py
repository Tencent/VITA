...
class FsmnLayer(nn.Module):
    def __init__(self, input_dim, out_dim, hidden_dim, left_frame=1, right_frame=1, left_dilation=1, right_dilation=1):
        ...
        if right_frame > 0:
            self.pad_right = nn.ConstantPad1d(
                [-right_dilation, right_dilation*right_frame], 0.0)
    ...
    def infer(self, x, buffer, buffer_index, buffer_out, hidden=None):
        ...
        cnn_buffer = buffer[buffer_index: buffer_index + self.buffer_size].reshape([1, self.hidden_dim, self.cache_size])
        ...
        if self.right_frame > 0:
            p_in_raw_cnn_buffer = buffer[buffer_index: buffer_index + self.p_in_raw_buffer_size].reshape([1, self.hidden_dim, self.p_in_raw_chache_size])
        ...
        if hidden is not None:
            if self.right_frame > 0:
                hidden_cnn_buffer = buffer[buffer_index: buffer_index + self.hidden_buffer_size].reshape([1, self.hidden_dim, self.hidden_chache_size])
