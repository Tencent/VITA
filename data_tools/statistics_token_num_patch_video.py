...
def get_video_frame(video_path, max_frames=MAX_IMAGE_LENGTH, min_frames=MIN_IMAGE_LENGTH, video_framerate=1, s=None, e=None):
    if s is None:
    ...
    if 'image' in item:
        ...
        for k, img_file in enumerate(image_file):
            if set_id[k] not in NoPatchSets:
                ...
                image = Image.open(os.path.join(image_directory, img_file.replace("\\", "/"))).convert('RGB')
