    ...
    for item in tqdm(data):
        ...
        for key in item.keys():
            if type(item[key]) is not int and key != 'id':
                assert len(item[key]) > 0, f"{input_file_name}, item {item} have null key!!!!!!!!!!{key}" 
        ...
        if count_image_path != count_image_ph: print(f"{input_file_name}, item {item} image place holder number NOT equal image file number")
        if count_video_path != count_video_ph: print(f"{input_file_name}, item {item} video place holder number NOT equal video file number")
        if count_audio_path != count_audio_ph: print(f"{input_file_name}, item {item} audio place holder number NOT equal audio file number")
        ...
        if "image" in item or "video" in item:
            if set_id[0] != 'sqa':
                assert len(set_id) == count_image_path+count_video_path, f"{input_file_name}, item {item} set_id numer Not correct"
